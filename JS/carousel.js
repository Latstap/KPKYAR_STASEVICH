document.addEventListener('DOMContentLoaded', function () {
  const track = document.querySelector('.feedback_content .carousel_feedback');
  const indicatorsContainer = document.querySelector('.feedback_content .carousel_indicators');
  const wrapper = document.querySelector('.feedback_content'); // контейнер для наблюдения
  let originalCards = Array.from(document.querySelectorAll('.feedback_content .carousel_feedback .feedback_card'));
  ...
  let allCards = Array.from(track.querySelectorAll('.feedback_card'));
  ...
  var indicators = Array.from(indicatorsContainer.querySelectorAll('span'));
  ...
  // Автопрокрутка — теперь управляем через Observer
  let timer = null;
